...
const ANIMAL_LINES = [
  ...
  },
  {
    ...
    desc: "A Swine Ant é uma solução tecnológica desenvolvida para auxiliar no manejo sustentável das instalações suinícolas, promovendo melhores condições ambientais, redução de odores e maior eficiência.",
...
export type AnimalLineId = "cattle" | "swine" | "poultry";

const LINE_ROUTES: Record<AnimalLineId, string> = {
  cattle: "/livestock-ant",
  swine: "/swine-ant",
  poultry: "/poultry-ant",
};
...
/* =========================================================================
   PÁGINA PRINCIPAL
   ========================================================================= */
export default function AnimalLinePage({ initialId = "cattle" }: { initialId?: AnimalLineId }) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const { scrollYProgress } = useScroll({ target: containerRef });
  const [showScrollTop, setShowScrollTop] = useState(false);
  
  // Estado para controlar a vitrine dinâmica de animais
  const [activeId, setActiveId] = useState<AnimalLineId>(initialId);
      ...
      <section className="relative w-full flex min-h-[auto] flex-col items-center overflow-hidden bg-[#0A0A0A] px-6 pb-14 pt-32 lg:min-h-[100svh] lg:flex-row lg:px-12 lg:py-0">
        ...
        <div className="w-full lg:w-1/2 relative z-20 flex flex-col justify-center lg:pr-12 xl:pl-12 mt-10 lg:mt-0">
          ...
          >
            ...
            <h1 className="text-[3rem] sm:text-5xl md:text-7xl lg:text-[5.5rem] font-serif font-normal uppercase tracking-normal sm:tracking-wide leading-[1.05] text-white mb-8 drop-shadow-lg">
              {activeLine.title.replace(" ANT", "")} <br />
            ...
            <p className="max-w-lg text-lg md:text-xl font-light leading-relaxed text-[#E0DED8] mb-12 border-l-2 border-[#D4AF37] pl-6 drop-shadow-md">
              {activeLine.headline} {activeLine.desc}
            </p>
            ...
            >
              Explorar Aplicações <ArrowRight size={16} />
            </Link>
          </motion.div>
        </div>
        ...
        <div className="relative z-10 mt-12 flex h-[340px] w-full items-center justify-center sm:h-[420px] lg:mt-0 lg:h-[100svh] lg:w-1/2">
          ...
          <div className="grid h-full w-full max-w-2xl grid-cols-3 gap-3 py-8 md:gap-5 lg:py-32">
            ...
            >
              <Image src="/images/cattle-showcase.webp" fill quality={75} className="object-cover transition-transform duration-1000 group-hover:scale-110 opacity-70 group-hover:opacity-100" alt="Livestock Ant" />
              ...
              <div className="absolute bottom-6 w-full flex flex-col items-center">
                ...
                <span className="text-[9px] font-sans font-semibold uppercase tracking-[0.2em] text-white drop-shadow-md">Livestock Ant</span>
              </div>
            </motion.div>

            {/* Pilar 2: Swine Ant (Destacado no centro) */}
            <motion.div 
              initial={{ opacity: 0, y: 80 }}
            ...
            >
              <Image src="/images/swine-showcase.webp" fill quality={75} className="object-cover transition-transform duration-1000 group-hover:scale-110 opacity-90 group-hover:opacity-100" alt="Swine Ant" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A0A] via-transparent to-transparent opacity-90" />
              <div className="absolute bottom-6 w-full flex flex-col items-center">
                <PiggyBank className="text-[#D4AF37] mb-2" size={28} strokeWidth={1.5} />
                <span className="text-[10px] font-sans font-semibold uppercase tracking-[0.2em] text-white drop-shadow-md">Swine Ant</span>
              </div>
            </motion.div>
            ...
            >
              <Image src="/images/poultry-showcase.webp" fill quality={75} className="object-cover transition-transform duration-1000 group-hover:scale-110 opacity-70 group-hover:opacity-100" alt="Poultry Ant" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A0A] via-transparent to-transparent opacity-90" />
              <div className="absolute bottom-6 w-full flex flex-col items-center">
                <Feather className="text-[#D4AF37] mb-2" size={24} strokeWidth={1.5} />
                <span className="text-[9px] font-sans font-semibold uppercase tracking-[0.2em] text-white drop-shadow-md">Poultry Ant</span>
              </div>
            </motion.div>
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-16">
            {ANIMAL_LINES.map((line) => {
              ...
              return (
                <Link
                  key={line.id}
                  href={LINE_ROUTES[line.id as AnimalLineId]}
                  onClick={() => setActiveId(line.id as AnimalLineId)}
      ...
      <section className="bg-[#1A1A1A] py-24 md:py-32 px-6 text-center border-t border-black">
        <div className="mx-auto max-w-4xl flex flex-col items-center">
          ...
          <p className="text-lg font-light text-[#888888] leading-relaxed mb-12 max-w-2xl">
            Da ambiência ao resíduo, a Anthars organiza soluções para operações
